...
func TestV1EventReportWithOptionalParams(t *testing.T) {
	t.Skip("Mock server tests are disabled")
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
	...
	_, err := client.V1.Events.Report(context.TODO(), stigg.V1EventReportParams{
		Events: []stigg.V1EventReportParamsEvent{{
			CustomerID:     "customerId",
			EventName:      "x",
			IdempotencyKey: "x",
			Dimensions: map[string]stigg.V1EventReportParamsEventDimensionUnion{
				"foo": {
					OfString: stigg.String("string"),
				},
			},
