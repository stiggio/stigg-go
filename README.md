...
```go
import (
	"github.com/stainless-sdks/stigg-go" // imported as stigg
)
```
...
```sh
go get -u 'github.com/stainless-sdks/stigg-go@v0.0.1-alpha.0'
```
...
The stigg library uses the [`omitzero`](https://tip.golang.org/doc/go1.24#encodingjsonpkgencodingjson)
semantics from the Go 1.24+ `encoding/json` release for request fields.

Required primitive fields (`int64`, `string`, etc.) feature the tag <code>\`json:"...,required"\`</code>. These
fields are always serialized, even their zero values.

Optional primitive types are wrapped in a `param.Opt[T]`. These fields can be set with the provided constructors, `stigg.String(string)`, `stigg.Int(int64)`, etc.
...
```go
p := stigg.ExampleParams{
	ID:   "id_xxx",            // required property
	Name: stigg.String("..."), // optional property

	Point: stigg.Point{
		X: 0,            // required field will serialize as 0
		Y: stigg.Int(1), // optional field will serialize as 1
		// ... omitted non-required fields will not be serialized
	},

	Origin: stigg.Origin{}, // the zero value of [Origin] is considered omitted
}
```
...
custom := param.Override[stigg.FooParams](12)
```
...
See the [full list of request options](https://pkg.go.dev/github.com/stainless-sdks/stigg-go/option).
...
`*stigg.Error`. This contains the `StatusCode`, `*http.Request`, and
...
We also provide a helper `stigg.File(reader io.Reader, filename string, contentType string)`
...
We are keen for your feedback; please open an [issue](https://www.github.com/stainless-sdks/stigg-go/issues) with questions, bugs, or suggestions.
