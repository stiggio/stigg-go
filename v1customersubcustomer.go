...
// V1CustomerSubCustomerService contains methods and other services that help with
// interacting with the stigg API.
//
// Note, unlike clients, this service does not read variables from the environment
// automatically. You should not instantiate this service directly, and instead use
// the [NewV1CustomerSubCustomerService] method instead.
type V1CustomerSubCustomerService struct {
	Options []option.RequestOption
}

// NewV1CustomerSubCustomerService generates a new service that applies the given
// options to each request. These options are applied after the parent client's
// options (if there is one), and before any request-specific options.
func NewV1CustomerSubCustomerService(opts ...option.RequestOption) (r V1CustomerSubCustomerService) {
	r = V1CustomerSubCustomerService{}
	r.Options = opts
	return
}
